...
library(psrccensus)
big_tbl <-get_decennial_recs(geography="tract", variables="DP1_0093C", years=2020, sumfile="dp")
```
...
map_it<-create_psrc_map(lyr=lyr_data, 
                        ...
                        legend_title= '2020 Hispanic Population',
...
```{r}
mapshot(map_it, url= "HispanicPop2020.html")
```


### Step 10. (optional) Save the map as png
```{r}
mapshot(map_it, file= "HispanicPop2020.png")

```


```{r, results=FALSE, message=FALSE, warning=FALSE}
big_tbl <-get_decennial_recs(geography="tract", variables="P005010", years=2010, sumfile="sf1")
```
...
### 7. Join to data table layer via matching IDs, choose the field to map
```{r }
lyr_data<- dplyr::left_join(lyr,tbl, by = c("geoid10"="GEOID"))
# this is the field to map
lyr_data_field<-lyr_data$Total
```
...
map_it<-create_psrc_map(lyr=lyr_data, 
                        lyr_data_field=lyr_data_field,
                        legend_title= '2010 Hispanic Population',
                        legend_subtitle = 'by Census Tract',
                        psrc_col_pal=psrc_orange_plus)
...
### Step 9. (optional) Save the map as html
```{r}
mapshot(map_it, url= "HispanicPop2010.html")
```


### Step 10. (optional) Save the map as png
```{r}
mapshot(map_it, file= "HispanicPop2010.png")
