...
tenure_prace <- psrc_pums_count(
  ...
  filter(tenure=="owner") %>%
  select(all_of(c("PRACE","share"))) %>%
  mutate(share = round(share*100,0))
...
tenure_subgroup <- psrc_pums_count(df_pums_aapi, group_vars=c("RAC2P_aapi_group10","tenure")) %>%
  ...
  select(all_of(c("RAC2P_aapi_group10","share")))
...
income <- psrc_pums_median(df_pums, group_vars="PRACE", stat_var = "HINCP") %>%
  ...
  mutate(order = case_when(RAC2P_aapi_group10 == "Region"~100,
                           ...
                           TRUE~row_number())) %>%
  select(all_of(c("RAC2P_aapi_group10","HINCP_median"))) %>%
  mutate(HINCP_median = round(HINCP_median,0))
...
# renter cost burden by aapi subgroups
renter_aapi <- psrc_pums_count(df_pums_renter_aapi, group_vars=c("RAC2P_aapi_group10","rent_pct_income_30")) %>% 
  # asian alone
  add_row(psrc_pums_count(df_pums_renter, group_vars=c("PRACE", "rent_pct_income_30")) %>% 
            rename(RAC2P_aapi_group10 = PRACE) %>%
            filter(RAC2P_aapi_group10 =="Asian alone")) %>% 
  # region
...
renter <- psrc_pums_count(df_pums_renter, group_vars=c("PRACE", "rent_pct_income_30")) %>% 
  # region
  add_row(psrc_pums_count(df_pums_renter, group_vars=c("rent_pct_income_30")) %>%
         mutate(PRACE = "Region", .after="COUNTY")) %>%
  filter(rent_pct_income_30 == "Greater than 30 percent") %>%
...
addWorksheet(wb, "cost burden (race)")
addWorksheet(wb, "cost burden")


writeData(wb, "tenure (race)", tenure_prace)
writeData(wb, "tenure", tenure_subgroup)
writeData(wb, "income", income)
writeData(wb, "cost burden (race)", renter)
writeData(wb, "cost burden", renter_aapi)
