...
```{r}
df_depress_more<-df_depress%>%filter(depress_grp=='More than half the days')%>%drop_na()
static_line_chart(df_depress_more, x='week', y='sum_total', fill='select_characteristics')
...
depress_anxiety<-rbind(df_anxiety_more_57, df_depress_more_57)%>%filter(select_characteristics %in% 
                                                                          c("    Yes"  , "    No" ))
```
...
```
```{r}
df_anxiety_more_avg<-df_anxiety_more%>%group_by(select_characteristics)%>%summarize(mean_share=mean(share), mean_total=mean(sum_total))%>%
 mutate(symptoms='anxiety')
df_depress_more_avg<-df_depress_more%>%group_by(select_characteristics)%>%summarize(mean_share=mean(share), mean_total=mean(sum_total))%>%
 mutate(symptoms='depression')
depress_anxiety<-rbind(df_anxiety_more_avg, df_depress_more_avg)%>%filter(select_characteristics %in% 
                                                                          c("    Yes"  , "    No" ))%>%mutate(LGBT=case_match(select_characteristics, 
                                          "    Yes" ~"LGBT identifying",
                                          "    No" ~ "Not LGBT identifying"))
```

```{r}
static_column_chart(depress_anxiety, x='symptoms',y='mean_share', fill= 'LGBT')

```

```{r}
depress_anxiety<-depress_anxiety%>%mutate(rounded_total=round(mean_total, -3))
static_column_chart(depress_anxiety, x='symptoms',y='rounded_total', fill= 'LGBT')
